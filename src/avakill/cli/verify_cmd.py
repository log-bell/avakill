"""AvaKill verify command -- verify a policy file's signature (auto-detects HMAC/Ed25519)."""
...
from avakill.core.integrity import _ED25519_PREFIX, FileSnapshot, PolicyIntegrity


@click.command()
@click.argument("policy_file")
@click.option("--key", default=None, help="Hex-encoded signing/verify key.")
...
def verify(policy_file: str, key: str | None, verbose: bool) -> None:
    """Verify a policy file's signature (auto-detects HMAC or Ed25519).

    Checks the .sig sidecar file against the policy contents.
    Uses AVAKILL_VERIFY_KEY for Ed25519, AVAKILL_POLICY_KEY for HMAC.
    """
    console = Console()
    ...
    # Auto-detect signature type to pick the right key
    sig_content = sig_path.read_text().strip()
    is_ed25519 = sig_content.startswith(_ED25519_PREFIX)

    if is_ed25519:
        key_hex = key or os.environ.get("AVAKILL_VERIFY_KEY")
        if not key_hex:
            console.print(
                "[red]Error:[/red] Ed25519 signature detected. "
                "Set AVAKILL_VERIFY_KEY or pass --key."
            )
            raise SystemExit(1)
        sig_type = "Ed25519"
    else:
