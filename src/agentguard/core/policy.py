...
from agentguard.core.exceptions import ConfigError, RateLimitExceeded
...
def load_policy(path: str | Path | None = None) -> PolicyEngine:
    ...
    If no path is given, looks for ``agentguard.yaml`` or
    ``agentguard.yml`` in the current working directory.
    ...
    raise ConfigError("No policy file found. Create an agentguard.yaml or pass an explicit path.")
