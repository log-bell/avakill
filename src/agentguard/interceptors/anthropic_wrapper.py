"""Anthropic interceptor that wraps an Anthropic client with AgentGuard policy checks."""
...
class _GuardedMessages:
    ...
    def _process_response(self, response: Any) -> Any:
        ...
        if not content:
            response.agentguard_decisions = []  # type: ignore[attr-defined]
            return response
        ...
        response.agentguard_decisions = evaluated  # type: ignore[attr-defined]
        return response
